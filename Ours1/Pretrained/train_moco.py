...
def parse_option():
    ...
    # IDR 'loss: separate two unsupervised processes', 'resnet not have cbr'
    parser.add_argument('--model_path2', type=str, default='./SaveModel2', help='path to save model')

    # MoCo 'loss: separate two unsupervised processes', 'resnet not have cbr'
...
def get_data_loader(args):
    ...
    obj = DataPreprocess()
    _, _, _, _, _, _, ground_xy_all_data = obj.data_about()
    ...
    # return get_loader.get_all_mark_data_loader(all_label=all_label, all_ground_xy=all_ground_xy)
...
def main2():
    ...
    # set the model_idr
    model_idr = set_model_idr(args=args)
    checkpoint = torch.load('../Partone/SaveModel2/ckpt_epoch_3_bei.pth', map_location='cpu')
