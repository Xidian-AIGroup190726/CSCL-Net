...
class CBR(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        # print('经过第一次卷积操作处理后的大小 {}: '.format(x.size()))
        x = self.bn1(x)
        x = self.relu(x)
        # x = self.maxpool(x)
        # x = self.avgpool(x)
...
class MoCo(nn.Module):
    ...
    def forward(self, j_pan_l, j_ms_l, j_pan_h, j_ms_h):
        ...
        # l_pos = torch.cat([l_pos_fp, l_pos_fm], dim=1)
        # print('the size of l_pos_fp is {}'.format(l_pos_fp.size()))
        # print('the size of l_pos_fm is {}'.format(l_pos_fm.size()))
        # print('the size of l_pos is {}'.format(l_pos.size()))
        ...
        l_neg = torch.cat([l_neg_fp, l_neg_fm], dim=1)  # 负样本 or 字典中，既有PAN 又有 MS
        # print('the size of l_neg_fp is {}'.format(l_neg_fp.size()))
        # print('the size of l_neg_fm is {}'.format(l_neg_fm.size()))
        # print('the size of l_neg is {}'.format(l_neg.size()))

        # total logits: Nx(1+K)
        # logits_fp = torch.cat([l_pos_fp, l_neg_fp], dim=1)
        # logits_fm = torch.cat([l_pos_fm, l_neg_fm], dim=1)
        logits_fp = torch.cat([l_pos_fp, l_neg], dim=1)  # query = fusion，pos = PAN， neg = PAN and MS
        logits_fm = torch.cat([l_pos_fm, l_neg], dim=1)  # query = fusion，pos = MS， neg = PAN and MS
        # apply temperature
        logits_fp /= self.T
        logits_fm /= self.T
